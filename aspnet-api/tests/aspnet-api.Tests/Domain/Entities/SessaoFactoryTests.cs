...
public class SessaoFactoryTests
{
    public class Create
    {
        ...
        public void DeveRetornarSucessoQuandoDadosForemValidos()
        {
            ...
            Assert.True(result.Data.EstaAtiva(DateTime.Now));
        }

        [Fact]
        public void DeveRetornarNotificacaoQuandoUsuarioIdForInvalido()
        {
            var result = Sessao.Create(0, Guid.NewGuid().ToString("N"), DateTime.Now.AddHours(1));
        ...
        {
            var result = Sessao.Create(1, jti, DateTime.Now.AddHours(1));
        ...
        {
            var result = Sessao.Create(1, Guid.NewGuid().ToString("N"), DateTime.Now.AddMinutes(-1));
    ...
    public class Revogar
    {
        ...
        public void DeveRevogarSessaoAtiva()
        {
            ...
            Assert.False(sessao.EstaAtiva(DateTime.Now));
        }
    ...
    public class EstaAtiva
    {
        ...
        {
            var sessao = Sessao.Create(1, Guid.NewGuid().ToString("N"), DateTime.Now.AddSeconds(1)).Data!;

            Assert.False(sessao.EstaAtiva(DateTime.Now.AddHours(1)));
        }
