...
const cartResponse = {
  ...
  data: {
    clienteId: 7,
    carrinhoId: 70,
    dataCarrinho: '2026-07-16T10:00:00Z',
    items: [
      {
        itemId: 701,
        produtoId: 42,
...
const createdOrderResponse = {
  ...
  data: {
    pedidoId: 900,
    clienteId: 7,
    dataPedido: '2026-07-16T12:00:00.000Z',
    formaPagamento: 'Pix',
    status: 'Criado',
    valorTotal: 399.8,
  },
...
describe('TASK-115 checkout integration', () => {
  ...
  it('posts strict confirmed contract once and applies exact 201 effects', async () => {
    ...
    const loads = { cart: 0, profile: 0 }
    server.use(
      ...baseHandlers(loads),
      http.post('*/api/v1/pedido', async ({ request }) => {
    ...
    expect(loads).toEqual({ cart: 1, profile: 1 })
    ...
    expect(bodies[0]).not.toHaveProperty('clienteId')
    expect(bodies[0]).not.toHaveProperty('carrinhoId')

    gate.resolve(
      HttpResponse.json(createdOrderResponse, { status: 201 }),
    )
    ...
    expect(
      queryClient.getQueryData(orderConfirmationKey(7, 900)),
    ).toEqual({
      id: 900,
      customerId: 7,
  ...
  it.each([409, 422])(
    'preserves checkout and skips success effects for HTTP %i',
    async (status) => {
      const loads = { cart: 0, profile: 0 }
      let posts = 0
      server.use(
        ...baseHandlers(loads),
        http.post('*/api/v1/pedido', () => {
          posts += 1
      ...
      expect(await screen.findByRole('alert')).toHaveTextContent(
        `Não foi possível confirmar o pedido${expectedCopy}`,
      )
      expect(posts).toBe(1)
      expect(
